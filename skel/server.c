...
*server_edit_document(server_t *s, char *doc_name, char *doc_content) {
    ...
    if (is == false) {
        ...
        ht_put(s->database, doc_name, MAX_RESPONSE_LENGTH,
                doc_content, MAX_RESPONSE_LENGTH);
    } else {
        // verificare database
        ...
        if (evicted_key) {
            sprintf(resp->server_log, LOG_EVICT, doc_name,
                    ((info *)evicted_key)->key);
            ht_put(s->database, ((info *)evicted_key)->key,
                    MAX_RESPONSE_LENGTH, ((info *)evicted_key)->value,
                    MAX_RESPONSE_LENGTH);
...
server_t *init_server(unsigned int cache_size) {
    ...
    server->database = ht_create(100, hash_string, compare_function_strings,
...
response *server_handle_request(server_t *s, request *req) {
    if (req->type == EDIT_DOCUMENT) {
        ...
        sprintf(resp->server_response, MSG_A, get_request_type_str(req->type),
                req->doc_name);
        ...
        while (s->task->size > 0) {
            elem = q_front(s->task);
            response *resp = server_edit_document(s, elem->doc_name,
                                                    elem->doc_content);
            PRINT_RESPONSE(resp);
            q_dequeue(s->task);
        }
